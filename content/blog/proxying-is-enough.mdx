...
description: "Recently Z. Luo et al released a paper titled “Proxying is Enough: Security of Proxying in TLS Oracles and AEAD Context Unforgeability” (Hash), where they discuss security of TLS Oracles."
date: 2024-05-29
tags: []
img: "/images/two.png"
...
Recently Z. Luo et al released a paper titled [“Proxying is Enough: Security of Proxying in TLS Oracles and AEAD Context Unforgeability”](https://eprint.iacr.org/2024/733.pdf) [(Hash)](https://etherscan.io/tx/0xa0c1f3d7a641a065688e148890a77f360b198e01e49aa28352e856682c4c12c9), where they discuss security of TLS Oracles.
...
From the paper for ChaCha20 : ![ChaCha Formula](https://blog.reclaimprotocol.org/images/zluo-6.1_corrected.png)
...
- $λ$ is the length of the string that can be revealed publicly without compromising privacy. Particularly, the set S of all possible values this string takes should be a finite set.
...
- $λ = 56*8$; for 56 bytes of padding, 8bits each; the paper uses the first two lines of the https response which contains the http status code and timestamp.
...
For ChaCha20, ![ChaCha Formula](https://blog.reclaimprotocol.org/images/zluo-6.1_corrected.png)

So, we’re looking at a minimum of 41 characters being used as padding. $λ = 41{bytes} = 328{bits}$
...
So, total valid openings is exactly $1$. $$ |S| = 1 $$

## Key size

Since we use chacha20, the IV is 12 bytes or 96 bits $$v = 96 $$

## Putting everything together

Feeding the above values in the formula, [we get probability](https://www.wolframalpha.com/input?i=log_10%282%2F2%5E%28%2814%2B17%2B5%29*8+-+2*96+%2B1%29%29) of breaking the security of Reclaim Protocol as $10^{-40} = 0.00000000000000000000000000000000000000001{\%}$
